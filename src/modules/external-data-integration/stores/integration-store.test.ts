...
describe('external data integration store', () => {
  ...
  it('loads and filters server-paged sync logs', async () => {
    ...
    await store.changeLogPageSize(50)
    expect(store.logPage).toBe(1)
    expect(store.logPageSize).toBe(50)
    expect(service.logCalls.at(-1)).toEqual({ query: { sourceId: '', result: 'fail' }, page: 1, pageSize: 50 })
    store.closeLogs()
