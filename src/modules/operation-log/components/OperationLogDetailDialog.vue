...
const formattedDetails = computed(() => props.log ? JSON.stringify(props.log.details, null, 2) : '')
const isLongDetails = computed(() =>
  formattedDetails.value.length > 900 || formattedDetails.value.split('\n').length > 18,
)
const targetText = computed(() => {
  if (!props.log) return '—'
  return `${props.log.targetType} ${props.log.targetId} · ${props.log.targetLabel}`
})
        ...
        <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <dl class="grid grid-cols-2 gap-x-6 gap-y-4 rounded-xl border bg-muted/25 p-4">
            <div>
              ...
              <dd class="mt-1.5 text-sm font-medium">{{ log.operatorName }} <span class="font-mono text-xs text-muted-foreground">({{ log.operatorUsername }})</span></dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd class="mt-1.5 text-sm font-medium">{{ OPERATION_LOG_MODULE_LABELS[log.module] }}</dd>
            </div>
            <div>
              <dt class="text-xs text-muted-foreground">操作类型</dt>
              <dd class="mt-1.5 text-sm font-medium">{{ OPERATION_LOG_ACTION_LABELS[log.action] }}</dd>
            </div>
            ...
            <div>
              <dt class="text-xs text-muted-foreground">日志编号</dt>
              <dd class="mt-1.5 font-mono text-sm text-primary">{{ log.code }}</dd>
            </div>
          </dl>
